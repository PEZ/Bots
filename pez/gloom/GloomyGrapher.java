...
//
// GloomyGrapher by PEZ
// Based on Kawigi's FloodGrapher
// http://robowiki.dyndns.org/?RoboGrapher
// $Id: GloomyGrapher.java,v 1.2 2003/12/06 23:27:41 peter Exp $

public class GloomyGrapher extends Canvas implements ActionListener, ItemListener, WindowListener {
    private static int[] segmentationsLengths = { 3, 5, 5, 1};
    ...
    public static void main(String args[]) {
        GloomyGrapher grapher = new GloomyGrapher();
        java.awt.Frame frame = new java.awt.Frame("Gloomy Grapher");
        frame.setSize(715, 540);
        folder = new File("GloomyDark.data/");
                ...
                for (s[2] = min[2]; s[2] <= max[2]; s[2]++) {
                    for (s[3] = min[3]; s[3] <= max[3]; s[3]++) {
                        ...
                        for (int bucket = 0; bucket < buckets; bucket++) {
                            graph[bucket] += stats[s[0]][s[1]][s[2]][s[3]][bucket];
                            samplesize += stats[s[0]][s[1]][s[2]][s[3]][bucket];
                            maxSamples = Math.max(maxSamples, stats[s[0]][s[1]][s[2]][s[3]][bucket]);
                        }
                    }
                }
            }
